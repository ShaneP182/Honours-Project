...
void PhysicsModel::GenerateSegments()
{
	...
	while (bIsGenerating)
	{
		...
		while (!bIsBranchFinished)
		{
			...
			{
				firstSegment = false;
				GenerateFirstSegment(segment, constA, lenDistribution(Rand_Generator)); // generate the segment
			...
			else
			{
				...
				segment.Temp = FMath::GetMappedRangeValueUnclamped(heightRange, tempRange, segment.StartPos.Z);

				// Calculate min diameter. dmin = (A * T) / p.
				segment.MinDiameter = (constA * (segment.Temp / 293.0f)) / segment.Pressure;
				...
				float diameter = FMath::Sqrt(0.5f) * (segment.Parent->Diameter / segment.Parent->MinDiameter) * segment.MinDiameter;

				// Get branching angle from normal distribution.
				float branchAngle = angleDistribution(Rand_Generator);

				// The angle is split between this segment, and the other segment in the branch.
				float splitAngle = branchAngle / 2;

				// Calculate the offset to add to the branching angle, so that the split is not always in the middle.
				float splitAngleOffset = FMath::FRandRange(-branchAngle / 2, branchAngle / 2);
				...
				segment.Length = lenDistribution(Rand_Generator) * segment.Diameter * Scale;
...
void PhysicsModel::GenerateFirstSegment(Segment& segment, float A, float lenDist)
{
	...
	segment.Temp = FMath::GetMappedRangeValueUnclamped(heightRange, tempRange, segment.StartPos.Z); // Unclamped, so even if height is out of bounds (i.e. below sea level), it will provide a temperature based on the trend.

	// Calculate the minimum diameter. The equation used here is: (p * dmin) / T = A [mm bar / 293K]. This can be re-arranged to dmin = (A * T) / p.
	segment.MinDiameter = ((A * (segment.Temp / 293.0f)) / segment.Pressure); // divide temperature by 293K to match A's units.
	...
	segment.Diameter = Voltage * ConstantnV;
	...
	segment.Length = lenDist * segment.Diameter * Scale;
